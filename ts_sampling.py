...
def plot_sample(
    ...
    for _ in range(samples):
        ...
        if state == "e":
            ...
            gradient, intercept, _, _, _ = stats.linregress(sample["year"], sample["U"])
    ...
    plt.title(
        f"{station} {N_above/len(sample_gradient):.2f}% samples above gradient difference criterion of {criterion:.2f} mm/yr with sample size {intervals}"
    )
    plt.plot(
        t,
        u,
    ...
    plt.plot(t, sample_fit[max], color="lightskyblue", label="Biggest outlier sample")

    plt.grid()
    plt.xlabel("Time [year]", fontsize=12)
    plt.ylabel("Up [mm]", fontsize=12)
    if state == "e":
        plt.ylabel("Easting [mm]", fontsize=12)
    plt.xlim([2002, 2020])
    plt.ylim([-20, 40])
    if state == "e":
        plt.ylim([2, 6])
    ...
    print(
        f"{station} {N_above/len(sample_gradient):.2f}% above criterion of {criterion:.2f} mm/yr"
    )
...
def plot_intervals(
    ...
    plt.plot(t, mean, "-", linewidth=0.75, color="crimson", label=f"Sample mean")

    plt.grid()
    plt.title(
        f"{(1-alpha)*100:.2f}% confidence intervals at {intervals[0:]} intervals in sampling"
    )
    plt.xlabel("Time [year]", fontsize=12)
    plt.ylabel("Up [mm]", fontsize=12)
    plt.xlim([2002, 2020])
...
if __name__ == "__main__":
    ...
    for station in data:
        ...
        plt.savefig(
            Path("out/sampling/single") / Path(f"{station}_sampling.png"),
            bbox_inches="tight",
        )
        ...
        plot_intervals(
            ...
            t,
            u,
            intervals=np.arange(3, 13, dtype=int),
            samples=1000,
            alpha=0.05,
        )
        plt.savefig(
            Path("out/sampling/multiple")
            / Path(f"{station}_variable_interval_sampling.png"),
            bbox_inches="tight",
        )
        ...
        plt.savefig(
            Path("out/sampling/easting") / Path(f"{station}_sampling_Ue.png"),
            bbox_inches="tight",
        )
